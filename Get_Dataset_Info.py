...
def num_reads(path, genome_dict, process_id):
    reads_dict = defaultdict(int)
    with open(f'results-process-id-{process_id}', 'w') as out_f:
        for label, genome_list in genome_dict.items():
            out_f.write(f'{label}\t{len(genome_list)}\t')
            for genome_fq in genome_list:
                ...
                reads_dict[label] += math.ceil(len(records) * .70)  # Only 70% of the reads are used for testing
...
def main():
    ...
    NCBI_training_path = sys.argv[1]
    fq_path = sys.argv[2]
    labels_dict = create_df(NCBI_training_path)
    list_label_dict = split_dict(labels_dict, mp.cpu_count())

    processes = [mp.Process(target=num_reads, args=(fq_path, list_label_dict[i], i)) for i in
