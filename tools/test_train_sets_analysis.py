...
def get_taxonomy(gtdb_info, test_genomes, train_genomes):
    ...
    taxonomy = [i.split(';')[-1].split('__')[1] for i in list(gtdb_df['gtdb_taxonomy'])]
    # get genome accession ids
    accession_id_list = [i[3:] for i in list(gtdb_df.accession)]
    # get taxonomy
    genomes_taxonomy = {i: taxonomy[i][::-1] if i in test_genomes or if i in train_genomes}
...
def get_avg_distance(test_genome, list_train_genomes, mash_dir, output_path, genomes_taxonomy):
    # get train genomes with same species as test genome
    train_genomes = [i for i in list_train_genomes if genomes_taxonomy[i] == genomes_taxonomy[test_genome]]
    print(f'Number of train genomes associated with test genome {test_genome}: {len(train_genomes)} - species: {genomes_taxonomy[test_genome]}')
    avg_msh_dist = float()
...
def main():
    ...
    output_path = os.path.join(mash_dir, 'mash-avg-dist-species')
    if job_id == 0:
        if not os.path.isdir(output_path):
            os.makedirs(output_path)
    # get average mash distance for each testing genome
    processes = [mp.Process(target=get_avg_distance, args=(test_g, train_genomes, mash_dir, output_path, genomes_taxonomy)) for test_g in test_genomes]
    for p in processes:
        p.start()
    for p in processes:
        p.join()
