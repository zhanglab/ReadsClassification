...
def write_paired_reads_to_file(reads, fw_reads, rv_reads):
...
def write_unpaired_reads_to_file(reads, fw_reads, rv_reads):
    num_reads_per_set = 500000
    total_num_reads = 0
    for i in range(0, len(reads), num_reads_per_set):
        subset_reads = []
        for r in reads[i:i+num_reads_per_set]:
            if r in fw_reads:
                subset_reads.append(fw_reads[r])
            elif r in rv_reads:
                subset_reads.append(rv_reads[r])
...
if __name__ == "__main__":
    ...
    with mp.Manager() as manager:
        ...
        write_unpaired_reads_to_file(unpaired_reads, fw_reads, rv_reads)
        write_paired_reads_to_file(paired_reads, fw_reads, rv_reads)
