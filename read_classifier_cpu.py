...
class DALIPreprocessor(object):
    def __init__(self, filenames, idx_filenames, batch_size, num_threads):
...
def main():
    ...
    results = pool.map(run_testing, izip(itertools.repeat(args, len(test_files)), itertools.repeat(results_dict, len(test_files)), test_files))
    pool.close()
    pool.join()
