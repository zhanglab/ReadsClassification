...
def AlexNet(VECTOR_SIZE, EMBEDDING_SIZE, NUM_CLASSES, VOCAB_SIZE, DROPOUT_RATE, output_dir=False):
    # define AlexNet model
    read_input = keras.layers.Input(shape=(VECTOR_SIZE), dtype='int32')
    x = read_input
    x = keras.layers.Embedding(input_dim=VOCAB_SIZE+1, output_dim=EMBEDDING_SIZE, embeddings_initializer=keras.initializers.HeNormal(),
                                          input_length=VECTOR_SIZE, mask_zero=True, trainable=True, name='embedding')(x)
    x = keras.layers.Reshape((VECTOR_SIZE, EMBEDDING_SIZE, 1))(x)
    x = keras.layers.Conv2D(96, kernel_size=(11, 11), strides=(4, 4), padding='same', kernel_initializer=keras.initializers.HeNormal(), name='conv_1')(x)
...
#    x = tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), padding='same')(x)
    x = keras.layers.Conv2D(256, kernel_size=(5, 5), strides=(1, 1), padding='same', kernel_initializer=keras.initializers.HeNormal())(x)
    x = keras.layers.BatchNormalization(axis=1,momentum=0.99)(x)
    x = keras.layers.Activation('relu')(x)
    x = keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), padding='same')(x)
    x = keras.layers.Conv2D(384, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_initializer=keras.initializers.HeNormal())(x)
    x = keras.layers.BatchNormalization(axis=1,momentum=0.99)(x)
    x = keras.layers.Activation('relu')(x)
    x = keras.layers.Conv2D(384, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_initializer=keras.initializers.HeNormal())(x)
    x = keras.layers.BatchNormalization(axis=1,momentum=0.99)(x)
    x = keras.layers.Activation('relu')(x)
    x = keras.layers.Conv2D(256, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_initializer=keras.initializers.HeNormal())(x)
    x = keras.layers.BatchNormalization(axis=1,momentum=0.99)(x)
    x = keras.layers.Activation('relu')(x)
    x = keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), padding='same')(x)
    x = keras.layers.Flatten()(x)
    x = keras.layers.Dense(units=4096, kernel_initializer=keras.initializers.HeNormal())(x)
    x = keras.layers.BatchNormalization(axis=1,momentum=0.99)(x)
    x = keras.layers.Activation('relu')(x)
...
#    x = tf.keras.layers.Dropout(0.5)(x)
    x = keras.layers.Dense(units=1000, kernel_initializer=keras.initializers.HeNormal())(x)
    x = keras.layers.BatchNormalization(axis=1,momentum=0.99)(x)
    x = keras.layers.Activation('relu')(x)
    x = keras.layers.Dropout(DROPOUT_RATE)(x)
    x = keras.layers.Dense(NUM_CLASSES, kernel_initializer=keras.initializers.HeNormal(), name='last_dense')(x)
    x = keras.layers.BatchNormalization(axis=1,momentum=0.99)(x)
    output = keras.layers.Activation('softmax', dtype='float32',)(x)
    model = keras.models.Model(read_input, output, name='AlexNet')
...
#        print(f'INDEX: {idx} - NAME: {model.get_layer(index = idx).name} - TYPE: {model.get_layer(index = idx).dtype}')
    if output_dir is True:
        with open(os.path.join(output_dir, f'model-alexnet.txt'), 'w+') as f:
